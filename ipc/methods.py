...
async def initUser(bot: commands.Bot, data: Dict) -> Dict:
    ...
    for mail in data.get("inbox"):
        ...
        mail["sender"] = {
            "avatarUrl": sender.display_avatar.url,
...
async def initPlayer(player: Player, member: Member, data: Dict) -> Dict:
    ...
    return {
        ...
        "filters": [
            {"tag": filter.tag, "scope": filter.scope, "payload": filter.payload}
            for filter in player.filters.get_filters()
        ],
...
async def updatePlaylist(bot: commands.Bot, data: Dict) -> Dict:
    ...
    if _type == "createPlaylist":
        ...
        if playlist_url:
            ...
            if not isinstance(tracks, Playlist):
                return {
                    "op": "updatePlaylist",
                    "status": "error",
                    "msg": "Please enter a valid link or public spotify or youtube playlist link.",
                    "field": "playlistUrl",
                    "userId": str(user_id),
                }
        ...
        data = (
            {"uri": playlist_url, "perms": {"read": []}, "name": name, "type": "link"}
            if playlist_url
            else {
                "tracks": [],
        ...
        if playlist["type"] in ["share", "link"]:
            return error_msg(
                "You cannot add songs to a linked playlist through Vocard.",
                user_id=user_id,
                level="error",
            )
        ...
        if len(playlist["tracks"]) >= max_t:
            return error_msg(
                f"You have reached the limit! You can only add {max_t} songs to your playlist.",
                user_id=user_id,
            )
        ...
        if decoded_track.is_stream:
            return error_msg(
                "You are not allowed to add streaming videos to your playlist.",
                user_id=user_id,
            )
        ...
        if playlist["type"] in ["share", "link"]:
            return error_msg(
                "You cannot remove songs from a linked playlist through Vocard.",
                user_id=user_id,
                level="error",
            )
        ...
        if not 0 <= track_position < len(playlist["tracks"]):
            return error_msg(
                "Cannot find the position from your playlist.",
                user_id=user_id,
                level="error",
            )
        ...
        if playlist["tracks"][track_position] != track_id:
            return error_msg(
                "Something wrong while removing the track from your playlist.",
                user_id=user_id,
                level="error",
            )
        ...
        if is_accept and len(list(user.get("playlist").keys())) >= max_p:
            return error_msg(
                f"You cannot create more than '{max_p}' playlists!",
                user_id=user_id,
                level="error",
            )
        ...
        for index, mail in enumerate(inbox.copy()):
            ...
            if is_accept:
                ...
                if refer_id not in share_playlists:
                    return error_msg(
                        "The shared playlist couldn’t be found. It’s possible that the user has already deleted it.",
                        user_id=user_id,
                    )
...
async def getSettings(bot: commands.Bot, data: Dict) -> Dict:
    ...
    if not guild:
        return error_msg(
            "Vocard don't have access to requested guild.",
            user_id=user_id,
            level="error",
        )
    ...
    if not member.guild_permissions.manage_guild:
        return error_msg(
            "You don't have permission to access the settings.",
            user_id=user_id,
            level="error",
        )
...
async def updateSettings(bot: commands.Bot, data: Dict) -> None:
    ...
    if not guild:
        return error_msg(
            "Vocard don't have access to required guild.",
            user_id=user_id,
            level="error",
        )
